...
public class ModAssignOperator implements AssignOperator {
    public static final ModAssignOperator INSTANCE = new ModAssignOperator();

    @Override
    public Value assign(Value leftSide, Value key, Value value) throws EvalError {
        var newValue = leftSide.getVar(key).mod(value);
